...
def main():
    ...
    try:
        ...
        string_block = String.load("postgres4")
        ...
        datetime2 = "select coalesce(max(start_at),'1970-01-01') from " + result[6] + '.' + result[7]
        datetime3 = "select coalesce(max(end_at),'1970-01-01') from " + result[6] + '.' + result[7]
        ...
        info_sql = "Select * from " + source_schema +'.' + source_table + ' where deleted_at > timestamp %s' + ' or start_at > timestamp %s' + \
            " or end_at > timestamp %s"
